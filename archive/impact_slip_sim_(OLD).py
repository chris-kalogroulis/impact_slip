...
class _BodySpeedFromState(LeafSystem):
    """
    Input:  plant state x = [q; v] (vector-valued)
    Output: scalar speed = || v_WB || for a chosen body B (world frame)
    """
    def __init__(self, *, plant: MultibodyPlant, body_name: str, model_instance=None):
        ...
        if model_instance is None:
            self._body = plant.GetBodyByName(body_name)
        else:
            self._body = plant.GetBodyByName(body_name, model_instance)

        self._nq = plant.num_positions()
        self._nv = plant.num_velocities()
        self._nx = self._nq + self._nv

        # Cached context we reuse every output calc (fast, avoids allocations).
    ...
    def _CalcSpeed(self, context, output):
        x = self.get_input_port(0).Eval(context)  # shape (nq+nv,)
        q = x[:self._nq]
        v = x[self._nq:]
...
def add_body_speed_logger(
    *,
    ...
    """
    Logs scalar speed of `body_name` using plant state output (vector-valued),
    so it works even when body_spatial_velocities port is abstract-valued.

    Returns:
      speed_logger (VectorLogSink)
    """
    speed_sys = builder.AddSystem(
        _BodySpeedFromState(plant=plant, body_name=body_name, model_instance=model_instance)
    )
    speed_sys.set_name(f"{body_name}_speed")

    builder.Connect(
        plant.get_state_output_port(),
        speed_sys.get_input_port(0),
    )
...
def plot_body_speed_logger(
    *,
    diagram,
    simulator: Simulator,
    speed_logger,
    title: str = "lower_leg speed",
    ylims=None,
):
    """
...
def create_scene(
    *,
    ...
    lower_leg_speed_logger = add_body_speed_logger(
        builder=builder, plant=plant, body_name="lower_leg"
    )
...
def simulate_impact_sliptest(
    *,
    ...
    return simulator, diagram, lower_leg_speed_logger
...
joint_params = {
    ...
    "knee":  dict(A="upper_leg", B="lower_leg",
                    k=500000.0, x0=0.40, d=3000.0),

    "ankle": dict(k=50.0, q0=0.00, d=3.0),
}
test_params = {
    "duration": 2.5,
    "x_pos0": 0.645,
...
if __name__ == "__main__":
    ...
    simulate_impact_sliptest(
        ...
        realtime_rate=1.0,
        rail_q0=test_params["x_pos0"],
        rail_v0=test_params["x_vel0"],
